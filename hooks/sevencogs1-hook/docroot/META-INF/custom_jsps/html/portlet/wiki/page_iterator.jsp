...
for (int i = 0; i < results.size(); i++) {
	...
	// Title
		...
		sb.append("<strong>");
		sb.append(curWikiPage.getTitle());
		sb.append("</strong>");

		if (!curWikiPage.isNew()) {
			sb.append("<br /><span style=\"font-size: 80%\">");

			if (type.equals("history")) {
				sb.append(LanguageUtil.get(pageContext, "edited-by"));
			}
			else {
				sb.append(LanguageUtil.get(pageContext, "last-edited-by"));
			}

			sb.append(StringPool.SPACE);
			sb.append(HtmlUtil.escape(PortalUtil.getUserName(curWikiPage.getUserId(), curWikiPage.getUserName())));
		}
