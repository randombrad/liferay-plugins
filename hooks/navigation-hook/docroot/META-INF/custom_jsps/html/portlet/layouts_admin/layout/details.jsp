...
<aui:fieldset>
	...
	<%
		String navigationPosition = selLayout.getTypeSettingsProperty("navigation-position","");
	%>

	<aui:select label="navigation-position" name="TypeSettingsProperties--navigation-position--">
	<%
	for (String navigationPositionOption : PropsUtil.getArray("navigation-hook-options")) {
	%>
		<aui:option selected='<%=navigationPosition.equals(navigationPositionOption) %>' label="<%= navigationPositionOption %>" />
	<%
	}
	%>
	</aui:select>
